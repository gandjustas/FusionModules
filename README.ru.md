...
Ранняя разработка, до 1.0. Публичная поверхность — один класс и шесть членов, зафиксированные
в `PublicAPI.Shipped.txt`: добавить туда что-то теперь можно только ревьюабельным изменением.

Не опубликовано: имя на nuget.org не занято, а занять его — решение без отмены, пока имя ещё
можно поменять (`Modulith` там тесно, плюс пересечение со Spring Modulith). `release.yml`
собирает пакет, проверяет его и прикладывает к GitHub-релизу; включить push — одна джоба и одно
решение. Пока ставится из локального фида:

```bash
dotnet pack src/Modulith/Modulith.csproj -c Release -o local-feed
```

Ровно так и сделано в [`gandjustas/dotnext-2026`](https://github.com/gandjustas/dotnext-2026) в
ветке `modulith-packages` — там пакет используется чем-то, внутри чего он не вырос.
